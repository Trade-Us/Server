...
def run_DQN():
    ...
    for e in range(episode):
        ...
        if mode == 'train' and (e + 1) % 10 == 0:  # checkpoint weights
            agent.save('weights/{}-b{}-e{}.h5'.format(timestamp, batch_size, episode))
    ...
    dqnresult = str(portfol_val[index][0]) +" "+ str(max_v)+"원 "+str(index+1)+"번째 에피소드"
...
def make_prediction():
    if request.method == 'POST':
        ...
        if not file: return render_template('index.html', labe="No Files")
